...
import { getEnvVar, logEnvConfigOnce } from './lib/getEnvVar';
...
const BookshelfScannerAppInner: React.FC = () => {
 ...
 console.log(` Cropping section: ${section.x}%, ${section.y}%, ${section.width}%, ${section.height}%`);
 ...
 console.log(` Client-side validation removed for security. Book "${book.title}" will be validated server-side.`);
 return book;
 };
 ...
 console.log(` Downgrading "${title}" by "${author}" - fake author`);
 confidence = 'low';
 }
 }
 ...
 const updatedPhotos = dedupBy([...photos, newPhoto], photoStableKey);
 ...
 if (!user) return;
 const { createDeleteIntent, assertDeleteAllowed, logDeleteAudit, isClearInProgress, setClearInProgress } = await import('./lib/deleteGuard');
 const { perfLog } = await import('./lib/perfLogger');
 if (isClearInProgress()) return;
 const tapAt = Date.now();
 perfLog('clear_library', 'tap', { tapAt, bookCount: 0, photoCount: 0 });
 setClearInProgress(true);
 const intent = createDeleteIntent('user_clear_library', 'App');
 intent.userConfirmed = true;
 if (!assertDeleteAllowed(intent, { rowCount: 1, isBulkConfirmed: true })) {
 ...
 // Server-side clear by user_id — do NOT rely on local counts. API runs UPDATE ... WHERE user_id = auth.uid().
 let booksUpdated = 0;
 let photosUpdated = 0;
 const { getApiBaseUrl } = await import('./lib/getEnvVar');
 const { getScanAuthHeaders } = await import('./lib/authHeaders');
 ...
 if (baseUrl) {
   try {
     ...
     console.warn('Clear library API error:', e);
   }
 }
 logDeleteAudit(intent, { rowCount: photosUpdated + booksUpdated, userId: user.uid, extra: { serverPhotosUpdated: photosUpdated, serverBooksUpdated: booksUpdated } });
 // Clear all library keys (same as Settings clear so MyLibraryTab stays empty)
 ...
 const { clearDeletedPendingStableKeys } = await import('./services/supabaseSync');
 ...
 const { setActiveScanJobIds } = await import('./lib/activeScanJobsStore');
 await setActiveScanJobIds(user.uid, []);
 setBooks([]);
 setPhotos([]);
 const stateCommittedAt = Date.now();
 perfLog('clear_library', 'state_committed', { stateCommittedAt, booksUpdated, photosUpdated });
 requestAnimationFrame(() => {
   perfLog('clear_library', 'list_rendered', { listRenderedAt: Date.now(), booksUpdated, photosUpdated });
 });
 } catch (e) {
 ...
 [
 { text: 'Cancel', style: 'cancel' },
 { text: 'Clear', style: 'destructive', onPress: clearLibrary },
 ]
 );
 };
 ...
 >
 <ChevronBackIcon size={24} color="#ecf0f1" />
 </TouchableOpacity>
