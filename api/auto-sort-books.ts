...
export default async function handler(req: VercelRequest, res: VercelResponse) {
 ...
 console.log(`[API] Auto-sorting ${books.length} books into collections...`);
 ...
 {
 role: 'user',
 content: `Sort these ${books.length} books into collections by their primary genre. Analyze each book's title and author to determine its genre.
...
4. NEVER create an "Other" collection. NEVER put books in "Other". If you can't match a book to an existing collection, create a new specific genre collection for it (e.g., "Poetry", "Art", "Cookbooks", etc.).
...
- folderName: The genre name (2-3 words max, no emojis). ${existingFolders && existingFolders.length > 0 ? 'Use EXACT existing collection names when matching. Only create new collection names for truly new genres.' : 'Use standard genre names like "Science Fiction", "Mystery", "Biography", "Business", "History", "Fiction", "Non-Fiction", etc.'}
- bookIds: Array of book IDs that belong in this genre folder

${existingFolders && existingFolders.length > 0 ? 'REMEMBER: Match EVERY book to an existing collection if possible. Be extremely flexible. Only create new collections for books that truly don\'t fit any existing genre. NEVER use "Other".' : 'Focus on genre classification. If a book\'s genre is unclear, use "Fiction" or "Non-Fiction" as appropriate. Only use "Other" for books where both title and author are "Unknown".'}
...
Return ONLY the JSON array, no explanations or markdown.`,
 ...
 console.error('[API] Failed to parse collection groups:', content);
 ...
 console.log(`[API] Created ${validGroups.length} collections from ${books.length} books`);
