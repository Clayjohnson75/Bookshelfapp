...
export default async function handler(req: VercelRequest, res: VercelResponse) {
 ...
 try {
 const { imageDataURL, jobId, photoId: bodyPhotoId } = req.body || {};
 const validPhotoId = typeof bodyPhotoId === 'string' && bodyPhotoId.trim() && UUID_REGEX.test(bodyPhotoId.trim()) ? bodyPhotoId.trim() : null;
 ...
 .eq('id', finalJobId)
 .maybeSingle();
 const existing = jobRow?.photo_id ?? null;
 if (existing != null && existing !== validPhotoId) {
 console.warn('[API] scan_jobs.photo_id already set to different id not overwriting (immutable)', { existing, requested: validPhotoId });
 } else if (existing == null) {
 const { error: patchErr } = await supabase
 .from('scan_jobs')
 .update({ photo_id: validPhotoId, updated_at: new Date().toISOString() })
 .eq('id', finalJobId)
 .is('photo_id', null);
 if (patchErr) {
 console.warn('[API] scan_jobs.photo_id patch failed (non-fatal):', patchErr.message);
 } else {
 console.log('[API] scan_jobs.photo_id set to', validPhotoId);
 }
 }
 }
 ...
 // Explicit pending/active query: not deleted, only active statuses, only recent, ordered, limited.
 // Sweeper has no userId (processes any user's jobs); user-facing list would add .eq('user_id', userId).
 const RECENT_DAYS = 7;
 const LIST_LIMIT = 10;
 ...
 .order('updated_at', { ascending: true }) // FIFO: process oldest-updated first
 .limit(LIST_LIMIT);
