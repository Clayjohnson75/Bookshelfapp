...
export default async function handler(req: VercelRequest, res: VercelResponse) {
 ...
 // Auth: Bearer token required. targetUserId is derived from token — never from body.
 ...
 try {
 const { transactionId, originalTransactionId, productId } = req.body;
 ...
 return res.status(401).json({ success: false, error: authErr?.message ?? 'Invalid or expired token' });
 }
 const targetUserId: string = userData.user.id;

 // TODO: Validate the receipt against Apple's servers before updating the subscription.
 // See the example implementation at the bottom of this file. Until then, only the
 // authenticated user's own subscription can be updated (IDOR is closed by token auth above).
