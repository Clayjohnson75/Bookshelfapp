...
export async function getScanAuthHeaders(): Promise<{ Authorization: string; 'Content-Type': string }> {
  ...
  if (__DEV__) {
    console.log('[SCAN_ENV]', JSON.stringify({ SUPABASE_URL, EXPO_PUBLIC_API_BASE_URL }, null, 2));
  }
  ...
  if (__DEV__) {
    console.log('[SCAN_AUTH_DEBUG]', JSON.stringify({
      hasSession: !!s,
      userId: s?.user?.id,
      accessTokenLen: at.length,
      looksLikeJwt: at.split('.').length === 3,
    }, null, 2));
  }
  ...
  if (__DEV__) {
    console.log('[SCAN_AUTH_HEADER]', { authPrefix: authHeader?.slice(0, 20), len: authHeader?.length });
  }
